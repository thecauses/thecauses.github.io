...
In causal inference, we are concerned with how a **treatment** causes some change in the **outcome** variable. The treatment variable is the one that causes something to occur, and that effect is reflected in the outcome variable:
...
We typically notate the treatment as $D$ for notation simplicity. As noted above, $D$ can take two values - you either got the treatment $D$, or did not get the treatment $D$.
...
The outcome variable is the result that changes as a result of either receiving or not receiving treatment. For example, your expected lifetime earnings change depending on if you went to college or not. Thus, your expected lifetime earnings would be an outcome in this scenario.
...
One way we can analyse the relationship between a treatment $D$ and an outcome $Y$ is with a correlation. **Correlation** is how one variable changes when the other variable changes.

For example, let us say that our treatment $D$ is whether or not you went to college. Our outcome $Y$ of interest is you career earnings.
...
-   The average health outcome $Y$ of individuals who went to the hospital (got the treatment) is [quite low]{.underline}.
-   The average health outcome $Y$ of individuals who did not go to the hospital (did not get the treatment) is [quite high]{.underline}.
...
We just discussed the idea of confounders - third variables that are correlated with our outcome $Y$, while also affecting who and who doesn't get the treatment $D$. We saw that the presence of confounders can result in our correlation being completely different from the causal effect.

But to dive deeper into causal inference, we need to formally define confounders. Confounders, often notated with a $X$, are variables that meet the following characteristics:

1.  The confounder $X$ is correlated (either positively or negatively) with the outcome variable $Y$ of interest.
2.  The confounder $X$ causes who gets and doesn't get the treatment $D$.
3.  The confounder is **not** itself caused by the treatment $D$.

We often explore confounders graphically:

![](images/clipboard-928665935.png){fig-align="center" width="40%"}

Above, we can see that $X$ is a confounder. It is correlated with outcome $Y$ (as shown by the arrow $X\rightarrow Y$ ), and it causes who gets and doesn't get the treatment $D$ (as shown by the arrow $X \rightarrow D$).

Note that $V$ is not a confounder. Why? $V$ does not cause who gets and doesn't get the treatment - note that the arrow is pointing from $D \rightarrow V$, meaning treatment $D$ is causing $V$, and not the other way around.

<br />

## Dealing with Confounders

We have discussed how the presence of confounders can result in our correlation being completely different from the causal effect. We can also visualise this graphically:

![](images/clipboard-928665935.png){fig-align="center" width="40%"}

The causal effect we want to find is the direct path $D \rightarrow Y$. When we estimate a correlation, we are estimating not only $D\rightarrow Y$, but also any other "path" through the confounder - in this case $D \leftarrow X \rightarrow Y$.

To get the true causal effect of $D\rightarrow Y$, we must find some way to "account" for confounder $X$. There are two main ways we do this in causal inference:

1.  We **control** for confounder $X$. This basically means we account for the influence of confounder $X$, and get rid of that part of influence.
2.  We **determine who gets treated**. For example, if we randomly decide who gets the treatment $D$ and who doesn't, that means **we the researchers** are controlling who gets $D$, and not the confounder $X$. Thus, we break the link between $X \rightarrow D$.
